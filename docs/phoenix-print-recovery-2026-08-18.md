...
Il riferimento a `_DEMON_ADAPTIVE_PA` riportato sopra descrive correttamente lo stato storico del profilo OrcaSlicer durante la fase di recovery del 18 agosto.

Il 23 agosto 2026 la causa è stata individuata nel campo `change_extrusion_role_gcode` dei quattro profili Phoenix attivi 0.2 / 0.4 / 0.6 / 0.8 nozzle.

La chiamata `_DEMON_ADAPTIVE_PA TYPE=[extrusion_role]` è stata rimossa da tutti e quattro i profili attivi.

Dopo il riavvio di OrcaSlicer e il nuovo slicing, il G-code generato contiene zero chiamate eseguibili `_DEMON_ADAPTIVE_PA`.

Questo hook non fa quindi più parte del workflow Phoenix corrente.
